  ...
  <path class="draw-split appear time-2" d="M10 20 L80 20 M10 40 L80 40 M10 60 L80 60"
      ...
      <ClassRef
        rows={[
          ...
          ["ease-NAME", "Any GSAP ease (default power3.out)"],
          ["delay / priority-N / .order", "Same spawn delay machinery"],
          [".scroll / .scroll-progress", "Enter-replay or scrubbed draw; leave reverses (split: back-to-front)"],
        ]}
      />
      <Code>{`<path class="draw scroll time-2 ease-power2.out" d="..." />
<path class="draw-split scroll-progress progress-start-20 progress-end-80" d="M10 20 L80 20 M10 40 ..." />`}</Code>

      <Note>
        Stroke only — ensure the target has <code>fill=&quot;none&quot;</code>{" "}
        and a visible <code>stroke</code>/<code>stroke-width</code>. Filled
        SVGs, markers and multi-path groups are untouched. Draw state is{" "}
        <code>drawSVG: 0% → 100%</code> with no opacity — the stroke IS the
        reveal. Splitting replaces the source path in the DOM and copies all
        attributes verbatim.
      </Note>
    </article>
  );
